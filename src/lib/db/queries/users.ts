...
export async function upsertUser(data: {
  ...
  const { rows } = await sql<User>`
    ...
    VALUES (${data.github_id}, ${data.github_login}, ${data.name}, ${data.avatar_url})
    ON CONFLICT (github_id)
    DO UPDATE SET github_login = ${data.github_login}, name = ${data.name}, avatar_url = ${data.avatar_url}
    RETURNING *
  `;
  return rows[0];
}
