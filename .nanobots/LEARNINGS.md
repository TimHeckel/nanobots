...
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); ran a distill pass
  ...
  maintainer replies on #18-22)
- **What worked / what didn't:** Sync found `main` CI red on the current head (`b756efb`,
  cycle 278's own docs-only commit), run
  [35856051696](https://github.com/TimHeckel/nanobots/actions/runs/35856051696) — only
  `onboarding-agent` failed, `test` passed. Diff confirmed docs-only via `gh api
  repos/.../commits/b756efb --jq '.files[].filename'` → exactly `.nanobots/LEARNINGS.md`.
  Original attempt: `FAILED 2 of 29` (`agent set PROJECTS_PAT`, `agent set the OCR endpoint
  variables`), no network-error text — the known non-network assertion cluster. `gh run
  rerun --failed` (watched via `gh run watch --exit-status`) came back green on both jobs on
  the very first rerun, breaking cycle 278's ninth-triple-non-clearing instance at a single
  ordinary clear. Posted as a dedupe comment on #21 (confirmed landed). No fresh P0; #22
  remains the mitigation, still unapproved. Re-verified cycle 278's own claims first: commit
  `b756efb` (docs-only, confirmed via the API), its #21 ninth-triple-instance comment, and
  its Status-issue report all checked out against live content. Pulled cycle 278's own
  outer-loop run (`35855499926`) denial count: **6**, ordinary band; posted to #19
  (confirmed landed). Checked #18/#20/#21/#22's full comment bodies (not just author — all
  are `TimHeckel` since that's the PAT identity) — all still the loop's own automated posts,
  no maintainer reply; #22 still exactly one comment (the plan), no `/nanobots start <hash>`
  line yet. Both crons healthy: outer's last 5 completed scheduled runs all success (this run
  in progress); worker's last 5 runs all success. No open PRs, no inbox items, no In
  Progress/In Review items (WIP 0/1). Undistilled LEARNINGS count (pre-append): suffix-
  anchored `[distilled]` count (Grep tool) → 141 against 151 total `^## ` headers (150 real
  entries after subtracting the format-template line) — 9 undistilled before this entry, 10
  after (this entry itself), crossing the ~10 threshold. Distill pass: promoted two things.
  Into TRIAGE.md's flake-judgment-refinements section — cycle 275's finding that the
  recurrence-dedupe treatment on #20 held even at a 13-run streak (~10x any prior recurrence)
  but the `show_full_output` recommendation should carry more weight in the dedupe comment
  when magnitude jumps an order of magnitude, even though the mechanical dedupe-not-refile
  decision itself doesn't change. Into RECIPES.md's "touching the GitHub API" recipe (as item
  14) — cycle 271's finding that `gh api repos/.../actions/runs/<id>/jobs` can report a job
  `in_progress` for tens of seconds after `completed_at` shows it already finished, so a
  slow-looking poll sequence isn't evidence of an unusually long run. Cycles 272, 273, 274,
  276, 277, 278 reviewed but yielded no new durable rule beyond what's already captured
  (clean-gap-doesn't-mean-resolved, single-triple-doesn't-set-a-floor, and
  cite-don't-restate-a-standing-proposal are all already promoted from earlier cycles) —
  marked `[distilled]` anyway per the same treatment cycle 270's pass used for confirmatory
  entries. Marked the 8 undistilled entries (cycles 271-278) `[distilled]`.
- **Lesson:** a distill pass over a run of mostly-confirmatory entries can still be worth
  running on schedule even when most individual entries add nothing new — the two real
  promotions here (#20's magnitude-escalation framing, the jobs-API poll-lag footgun) were
  each buried inside an entry whose headline was "ordinary clear" or "quiet cycle," and would
  have kept aging undistilled indefinitely if the pass were skipped just because the recent
  entries "looked routine" at a glance. This is the same failure mode TRIAGE.md's
  chore-threshold lesson and the missed-fourth-instance-trigger lesson already name for other
  "repeatedly" triggers — a threshold crossing needs to be checked by the number, not by
  whether the surrounding evidence *reads* as eventful.
- **Applies to:** triage | verify

## 2026-09-23 — cycle 278: #21 ninth triple-non-clearing instance (original + 2 reruns all red), #19 denial-count data point (cycle 277 run = 5) [distilled]
...
## 2026-09-23 — cycle 277: #21 ordinary single-rerun clear (on cycle 276's own commit), #19 denial-count data point (cycle 276 run = 1) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-22)
...
## 2026-09-22 — cycle 276: quiet cycle — outer cron healthy again, main CI green on first attempt on bc6ffc7, #19 denial-count data point (cycle 275 run = 11) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-22)
...
## 2026-09-22 — cycle 275: #20 recurrence at 13 consecutive scheduled failures (~2.6 days), by far the largest since the original 28-run filing [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-22)
...
## 2026-09-19 — cycle 274: #21 ordinary single-rerun clear (breaks the eighth-triple streak at one), #19 denial-count data point (cycle 273 run = 7) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-22)
- **What worked / what didn't:** Sync found `main` CI red on the current head (`42c7802`,
  cycle 273's own docs-only commit), run
  [35439100316](https://github.com/TimHeckel/nanobots/actions/runs/35439100316) — only
  `onboarding-agent` failed, `test` passed. Diff confirmed docs-only via `gh api
  repos/.../commits/42c7802 --jq '.files[].filename'` → exactly `.nanobots/LEARNINGS.md`.
  Original attempt: `FAILED 5 of 29` (`PROJECTS_PAT`, `DAYTONA_API_KEY`, OCR endpoint
  variables, `verify_daytona`, verify-before-store ordering), no network-error text — the
  known non-network assertion cluster. `gh run rerun --failed` (watched via `gh run watch
  --exit-status`) came back green on both jobs on the very first rerun, breaking cycle 273's
  eighth-triple-non-clearing instance at a single ordinary clear rather than a ninth
  consecutive or paired triple. Posted as a dedupe comment on #21 (confirmed landed). No
  fresh P0; #22 remains the mitigation, still unapproved. Re-verified cycle 273's own claims
  first: commit `42c7802` (docs-only, matches), its #21 dedupe comment, and its Status-issue
  report all checked out against live content. Pulled cycle 273's own outer-loop run
  (`35438849468`) denial count: **7**, ordinary band; posted to #19 (confirmed landed).
  Checked #18/#20/#21/#22's last comment bodies in full — still the loop's own automated
  posts, no maintainer reply; #22 still exactly one comment (the plan), no `/nanobots start
  <hash>` line yet. Both crons healthy: outer's last 5 completed scheduled runs all success
  (this run in progress); worker's last 5 runs all success. No open PRs, no inbox items, no
  In Progress/In Review items (WIP 0/1). Undistilled LEARNINGS count (pre-append): `grep -c
  "^## "` → 146 headers (145 entries), suffix-anchored `[distilled]` count (Grep tool) →
  141 — 4 undistilled before this entry, 5 after; well under the ~10 threshold, no distill
  pass this cycle.
- **Lesson:** the "clean gap doesn't mean the pattern stopped" framing from cycle 273 cuts
  both ways — a single triple instance also doesn't establish a new floor; the very next
  push cleared on the first rerun, same as the ordinary-clear majority of pushes tracked so
  far. Keep reading each recurrence (or non-recurrence) as one more data point in an
  already-established distribution, not as confirmation of a trend in either direction.
- **Applies to:** triage | verify

## 2026-09-19 — cycle 273: #21 eighth triple-non-clearing instance (recurs after 3-cycle clean gap), #19 denial-count data point (cycle 272 run = 2) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-22)
...
## 2026-09-19 — cycle 272: quiet cycle — main CI green on first attempt on ddb93a7 (no rerun needed), #19 denial-count data point (cycle 271 run = 5) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-22)
...
## 2026-09-18 — cycle 271: #21 ordinary single-rerun clear (second in a row, breaks any incipient streak at zero), #19 denial-count data point (cycle 270 run = 4) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-22)
...
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); ran a distill pass
  ...
  (8 Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-21)
- **What worked / what didn't:** Sync found `main` CI red on the current head (`378f0f8`,
  cycle 259's own docs-only commit), run 35057488128 — only `onboarding-agent` failed, `test`
  passed. Diff confirmed docs-only via `gh api .../commits/378f0f8 --jq '.files[].filename'`
  → exactly `.nanobots/LEARNINGS.md`. Original attempt: `FAILED 5 of 29` (`agent set a model
  credential (saw: PROJECTS_PAT, OCR_LLM_TOKEN)`, `agent set DAYTONA_API_KEY`, `agent set the
  OCR endpoint variables`, `agent ran verify_daytona`, `agent verified Daytona BEFORE storing
  the key`) — no network-error text, known non-network assertion cluster. First `gh run rerun
  --failed` (watched via `gh run watch --exit-status`) stayed red: `FAILED 1 of 29` (`agent
  set PROJECTS_PAT`) — still no network-error text. Second rerun came back green on both jobs.
  This is the ordinary double-non-clearing-then-clear shape — it breaks the three-consecutive
  triple-non-clearing streak (cycles 257, 258, 259, each red on original + both reruns) right
  on the next push, confirming the triple shape is a recurring possibility layered on the
  double pattern rather than a new floor. Posted as a dedupe comment on #21 (confirmed
  landed), and folded the triple-streak observation into a new TRIAGE.md distilled bullet
  rather than just logging it here again. No fresh P0; #22 remains the mitigation, still
  unapproved. Re-verified cycle 259's own claims first: commit `378f0f8` (docs-only, matches),
  its #21 and #19 comments both checked out against live content. Pulled cycle 259's own
  outer-loop run (`35057065205`) denial count: **12**, ordinary band; posted to #19 (confirmed
  landed). Checked #18/#20's last comment bodies in full (not just author) — both still the
  loop's own automated reports, no maintainer reply. Checked #22 — still exactly one comment
  (the plan), no `/nanobots start <hash>` line yet. Both crons healthy: outer's recent
  scheduled runs all success (this run in progress); worker's last five runs all success. No
  open PRs, no inbox items, no In Progress/In Review items (WIP 0/1). Undistilled LEARNINGS
  count (pre-append): `grep -c "^## "` → 133 headers (132 entries), suffix-anchored
  `[distilled]` count → 132 (after marking cycles 250-259 `[distilled]` this cycle) — 0
  undistilled before this entry, 1 after. Distill pass: promoted the triple-non-clearing
  pattern (cycles 257-260) into a new TRIAGE.md bullet, and the partial-trace-gap
  generalization (cycle 253/254's finding) into a new RECIPES.md point in the
  "verifying a cycle's own claims" recipe; marked the 9 undistilled entries (cycles 250, 251,
  252, 254, 255, 256, 257, 258, 259) `[distilled]`.
- **Lesson:** a threshold trigger phrased as "~10 undistilled entries" is easy to keep
  deferring one cycle at a time when the undistilled count creeps up by exactly one per
  cycle (as it has here) — checking the actual `grep` count against the threshold every
  cycle, not just when something already looks close, is what caught this one at exactly the
  boundary (9 before this entry, the entry that would make it 10) rather than a cycle or two
  past it. Both TRIAGE.md and RECIPES.md still had durable, un-promoted lessons sitting in
  undistilled LEARNINGS entries even though the underlying facts (the tally, the trace-gap
  variant) had already been individually noted — distilling them into the policy docs makes
  them findable without reading nine consecutive near-identical LEARNINGS entries.
- **Applies to:** triage | verify

## 2026-09-16 — cycle 259: #21 third consecutive triple-non-clearing instance (257, 258, 259), #19 denial-count data point (cycle 258 run = 0) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-21)
...
## 2026-09-15 — cycle 258: #21 second consecutive triple-non-clearing instance (back-to-back with cycle 257's first), #19 denial-count data point (cycle 257 run = 2) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-21)
...
## 2026-09-15 — cycle 257: #21 first-ever triple-non-clearing instance (original + 2 reruns all red), #19 denial-count data point (cycle 256 run = 5) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-21)
- **What worked / what didn't:** Sync found `main` CI red on the current head (`ce4ec78`,
  cycle 256's own docs-only commit), run 34965152656 — only `onboarding-agent` failed, `test`
  passed. Diff confirmed docs-only via `gh api .../commits/ce4ec78 --jq '.files[].filename'`
  → exactly `.nanobots/LEARNINGS.md`. Original attempt: `FAILED 4 of 29` (model credential,
  `PROJECTS_PAT`, +2 more), no network-error text — the known non-network assertion cluster.
  First `gh run rerun --failed` (watched via `gh run watch --exit-status`) stayed red:
  `FAILED 6 of 29`, a wider subset, still no network-error text. Second rerun *also* stayed
  red: `FAILED 1 of 29` (`agent set PROJECTS_PAT`), narrower again, still no network-error
  text. This is the first time all three attempts on one commit (original + both reruns)
  stayed red — every prior instance cleared somewhere in that window (a single-attempt clear,
  or the established "double-non-clearing" pattern where the second rerun clears). Stopped at
  two reruns per the standing two-attempt evidence-gathering bound rather than trying a third,
  and posted this as a dedupe comment on #21 (confirmed landed) — no fresh P0, #22 remains the
  mitigation and still has no `/nanobots start` approval. Running tally: 14
  double-or-worse-non-clearing instances (cycles 233, 236, 238-retroactive, 239, 240, 242, 244,
  245, 246, 249, 251, 254, 255, 257) against four single-attempt clears (247, 248, 250, 252)
  and cycle 256's one fully-clean-on-original-push observation. Re-verified cycle 256's own
  claims first: commit `ce4ec78` (docs-only, matches), its #19 comment and Status-issue report
  both checked out against live content. Pulled cycle 256's own outer-loop run (`34964874848`)
  denial count: **5**, ordinary band; posted to #19 (confirmed landed). Checked #18/#20's last
  comment bodies (not just author) — both still the loop's own automated posts, no maintainer
  reply. Both crons healthy: outer's recent scheduled runs all success (this run in progress);
  worker's last runs all success. No open PRs, no inbox items, no In Progress/In Review items
  (WIP 0/1). Undistilled LEARNINGS count (pre-append): `grep -c "^## "` → 130 headers (129
  entries), suffix-anchored `[distilled]` count → 123 — 6 undistilled before this entry, 7
  after; still under the ~10 distill threshold.
- **Lesson:** the double-non-clearing pattern this file has tracked since cycle 233 was itself
  a floor, not a ceiling — a triple-non-clearing instance was always possible and this cycle
  is the first observed case. Per the standing "propose, don't silently deviate" instinct, this
  is surfaced as new evidence (and additional weight behind #22's recommendation), not grounds
  to unilaterally extend the retry bound to three attempts as a new default. If a triple (or
  worse) recurs, that pattern — not a single instance — would be the trigger to reconsider the
  two-attempt bound explicitly.
- **Applies to:** triage | verify

## 2026-09-15 — cycle 256: quiet cycle — main CI clean on first attempt, no #21 dedupe needed, #19 denial-count data point (cycle 255 run = 3) [distilled]
- **Outcome:** n/a (Sync-time check, not a dispatched item); board unchanged (8 Done/4
  Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-21)
...
## 2026-09-15 — cycle 255: #21 thirteenth double-non-clearing instance, #19 denial-count data point (cycle 254 run = 3) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-21)
...
## 2026-09-14 — cycle 254: #21 twelfth double-non-clearing instance, #19 denial-count data point (cycle 253 run = 7) + a new partial-trace-gap variant [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-21)
...
## 2026-09-14 — cycle 252: #21 twelfth-overall / fourth single-attempt clear (new assertion sub-shape), #19 denial-count data point (cycle 251 run = 3) [distilled]
- **Outcome:** n/a (Sync-time judgment call, not a dispatched item); board unchanged (8
  Done/4 Blocked/1 Ready, #22 still Ready with a plan but no `/nanobots start` approval, no
  maintainer replies on #18-21)
...
## 2026-09-14 — cycle 251: #21 eleventh double-non-clearing instance, #19 denial-count data point (cycle 250 run = 4) [distilled]
...
## 2026-09-13 — cycle 250: #21 single-rerun clear (does not extend cycle 249's resumed streak), #19 denial-count data point (cycle 249 run = 7) [distilled]
...
- **What worked / what didn't:** Sync found `main` CI red on the current head (`4d7c0df`,
  cycle 246's own docs-only commit), run 34738895395 — only `onboarding-agent` failed, `test`
  passed. Diff confirmed docs-only via `gh api .../commits/4d7c0df --jq
  '.files[].filename'` → exactly `.nanobots/LEARNINGS.md`. Original attempt `FAILED 6 of 29`
  (`agent set a model credential`, `agent set PROJECTS_PAT`, `agent set DAYTONA_API_KEY`,
  `agent set the OCR endpoint variables`, `agent ran verify_daytona`, `agent verified Daytona
  BEFORE storing the key`) — same known non-network assertion cluster as the prior nine
  instances. One `gh run rerun --failed` (the established two-attempt bound, via `gh run
  watch --exit-status`) came back **fully green** this time — both jobs passed on the single
  rerun, breaking the nine-instance double-non-clearing streak (cycles 233, 236,
  238-retroactive, 239, 240, 242, 244, 245, 246). Posted as a comment on #21 rather than
  treating one clear as a reason to revise the "rate is durably higher" conclusion — one data
  point in either direction isn't enough to move a conclusion nine instances built. No board
  work waiting (0 open PRs, 0 inbox items, no Ready/In Progress/In Review items). Re-verified
  cycle 246's claims against live state first: its commit (confirmed via the GitHub API file
  list), #21 comment, and #19 comment all checked out exactly as reported. Pulled cycle 246's
  own outer-loop run (`34738715534`) denial count: **2**, within the established 0-14 range,
  ...
  healthy: outer's recent scheduled runs all success; worker's last 5 all success. Recomputed
  the undistilled count post-append: 122 total headers (121 entries), 113 `[distilled]` ⇒
  **8** undistilled — still below the ~10 threshold, no distill pass this cycle.
- **Lesson:** the double-non-clearing streak was itself only ever framed as "thin evidence
  the rate is higher, not proof" — the correct symmetric treatment of a clean break is "thin
  evidence against," not "streak over, revert to the original two-attempt assumption." Keep
  logging every instance, clearing or not, so the actual clear-rate (not just the streak
  length) stays visible to whoever eventually looks at `tests/init-agent.e2e.mjs`. Also: the
  standing verification/dedupe/denial-count checklist ported cleanly to a cycle whose central
...
- **What worked / what didn't:** Sync found `main` CI red on the current head (`142d93d`,
  cycle 245's own docs-only commit), run 34724588508 — only `onboarding-agent` failed, `test`
  passed. Diff confirmed docs-only via `gh api .../commits/142d93d --jq '.files[].filename'`
  → exactly `.nanobots/LEARNINGS.md`. Original attempt `FAILED 5 of 29` (`agent set
  PROJECTS_PAT`, `agent set DAYTONA_API_KEY`, `agent set the OCR endpoint variables`, `agent
  ran verify_daytona`, `agent verified Daytona BEFORE storing the key`); one `gh run rerun
  --failed` (the established two-attempt bound, via `gh run watch --exit-status`) did not
  clear — `FAILED 1 of 29`, a smaller subset of the same known non-network cluster (`agent
  set the OCR endpoint variables`). Both attempts stayed non-network throughout, so condition
  2 of the flake exception fails on both. Posted as a comment on #21 per the issue-agnostic
  dedupe rule (confirmed landed) — ninth double-non-clearing-rerun instance now (cycles 233,
  236, 238-retroactive, 239, 240, 242, 244, 245, 246), 8 of the last 9 `main` pushes needing
  a non-clearing rerun. Did not attempt a third rerun or widen condition 2/the two-attempt
  bound — no board work waiting either (0 open PRs, 0 inbox items, no Ready/In Progress/In
  Review items). Re-verified cycle 245's claims against live state first: its commit
  (confirmed via the GitHub API file list), #21 comment, and #19 comment all checked out
  exactly as reported. Pulled cycle 245's own outer-loop run (`34724371407` — not
  `34717420106`, which is cycle 244's run; double-checked the cron run list by timestamp
  before pulling) denial count: **4**, within the established 0-14 range, ordinary band;
  posted to #19 (confirmed landed). Checked #18/#20's last comment bodies (not just author)
  — both still the loop's own automated posts, no maintainer reply. Both crons healthy:
  outer's last 5 scheduled runs (this one in progress, 245, 244, 243, 242) all success;
  worker's last 5 all success. Recomputed the undistilled count post-append: 121 total
  headers (120 entries), 113 `[distilled]` ⇒ **7** undistilled — below the ~10 threshold, no
  distill pass this cycle.
- **Lesson:** the running-total framing continues to hold — each new instance is one more
  ...
  own to unilaterally widen the mechanical rule. Nine instances across the last ~13-14 pushes
  to `main` (8 of the last 9) keeps the rate materially higher than the "clears within two
  attempts" assumption the exception was originally written around. Also: when pulling a
  named cycle's own run ID for the denial-count metric, match by timestamp against the cron's
  run list explicitly rather than assuming the most-recently-referenced run ID in a prior
  comment is the right one — the run IDs for consecutive cycles are easy to transpose by one
  position, exactly the off-by-one slip RECIPES.md's metric-citation rule already warns
  about.
- **Applies to:** triage | verify
...
- **What worked / what didn't:** Sync found `main` CI red on the current head (`a1f656a`,
  cycle 243's own docs-only commit), run 34702606693 — only `onboarding-agent` failed, `test`
  passed. Diff confirmed docs-only via `gh api .../commits/a1f656a --jq '.files[].filename'`
  → exactly `.nanobots/LEARNINGS.md`. Original attempt `FAILED 1 of 29` (`agent set the OCR
  endpoint variables`); one `gh run rerun --failed` (the established two-attempt bound, via
  `gh run watch --exit-status`) did not clear — `FAILED 4 of 29`, a different subset of the
  same known non-network cluster (`agent set a model credential`, `agent set
  DAYTONA_API_KEY`, `agent ran verify_daytona`, `agent verified Daytona BEFORE storing the
  key`). Both attempts stayed non-network throughout, so condition 2 of the flake exception
  fails on both. Posted as a comment on #21 per the issue-agnostic dedupe rule (confirmed
  landed) — seventh double-non-clearing-rerun instance now (cycles 233, 236,
  238-retroactive, 239, 240, 242, 244), 6 of the last 7 `main` pushes needing a non-clearing
  rerun. Did not attempt a third rerun or widen condition 2/the two-attempt bound — no board
  work waiting either (0 open PRs, 0 inbox items, no Ready/In Progress/In Review items).
  Re-verified cycle 243's claims against live state first: its commit (confirmed via the
  GitHub API file list), #21 comment, and #19 comment all checked out exactly as reported.
  Pulled cycle 243's own outer-loop run (`34702491286`) denial count: **1**, tied with the
  recent lows (206, 210, 241, 242), not a new extreme; posted to #19 (confirmed landed).
  Checked #18/#20's last comment bodies (not just author) — both still the loop's own
  automated posts, no maintainer reply. Both crons healthy: outer's last 5 scheduled runs
  (this one in progress, 243, 242, 241, 240) all success; worker's last 5 all success.
  Recomputed the undistilled count post-append: 119 total headers (118 entries), 113
  `[distilled]` ⇒ **6** undistilled — well below the ~10 threshold, no distill pass this
  cycle.
- **Lesson:** the running-total framing continues to hold — each new instance is one more
  data point toward "a human should look at `tests/init-agent.e2e.mjs`," not grounds on its
  own to unilaterally widen the mechanical rule. Seven instances across the last ~11-12
  pushes to `main` (6 of the last 7) keeps the rate materially higher than the "clears within
  two attempts" assumption the exception was originally written around, and this instance's
  rerun again failed a *different specific subset* of the same known cluster rather than
  repeating the original assertion — consistent with live-model nondeterminism, not a
...
- **Outcome:** n/a (standing metric pull, not a dispatched item; board unchanged — 8/12 Done,
  #18-21 still `summon-human`/Blocked, no maintainer replies on any, checked each issue's
  actual last comment body per RECIPES.md's author-alone-isn't-enough rule)
- **What worked / what didn't:** re-verified cycle 224's report against live state first —
  commit `d8496cd` confirmed via `gh api .../commits/d8496cd...--jq '.files[].filename'` to
  touch exactly `.nanobots/LEARNINGS.md`, matching the docs-only data-point claim. Its own
  #19 and #21 comments both confirmed landed (1 comment each after 2026-09-08T16:00:00Z).
  Board, inbox, and PR state all matched cycle 224's report exactly (0 inbox, 0 open PRs, no
  Ready/In Progress/In Review items). `main` CI green on `d8496cd`; both crons healthy (outer:
  last 5 scheduled runs all success, this run itself in progress; worker: last 5 all success,
  most recent run `34252090423` logged "no claimable Ready item this run"). No new #20/#21
  recurrence since cycle 224 — no red outer-loop or main-CI runs found in the window. Pulled
  cycle 224's own outer-loop run (`34252246743`) denial count via `gh run view <id> --log |
  grep permission_denials_count`: **6**, ordinary band (established range 0-14).
- **Lesson:** no change to the standing conclusion — the count's magnitude still hasn't
  correlated with report accuracy in either direction. Continuing to track per RECIPES.md's
  standing instruction until #19's root cause lands.
- **Applies to:** verify
...
- **Outcome:** n/a (standing metric pull, not a dispatched item; board unchanged — 8/12 Done,
  #18-21 still `summon-human`/Blocked, no maintainer replies on any, checked each issue's
  actual last comment body per RECIPES.md's author-alone-isn't-enough rule)
- **What worked / what didn't:** re-verified cycle 222's report against live state first —
  commit `7963e379` confirmed via `gh api .../commits/7963e379...--jq '.files[].filename'` to
  touch exactly `.nanobots/LEARNINGS.md`, matching the docs-only data-point claim. Board,
  inbox, and PR state all matched cycle 222's report exactly (0 inbox, 0 open PRs, no
  Ready/In Progress/In Review items). `main` CI green on `7963e379`; both crons healthy (outer:
  last 5 scheduled runs all success; worker: last 5 all success, most recent run
  `34199434240` logged "no claimable Ready item this run", consistent with an empty Ready
  column). Pulled cycle 222's own outer-loop run (`34187548351`) denial count via `gh run view
  <id> --log | grep permission_denials_count`: **1**, ordinary band (established range 0-14).
  Posted this data point directly as a `gh issue comment` on #19, confirmed landed by reading
  it back.
- **Lesson:** no change to the standing conclusion — the count's magnitude still hasn't
...
- **Outcome:** n/a (standing metric pull, not a dispatched item; board unchanged — 8/12 Done,
  #18-21 still `summon-human`/Blocked, no maintainer replies on any, checked each issue's
  actual last comment body per RECIPES.md's author-alone-isn't-enough rule)
- **What worked / what didn't:** re-verified cycle 218's report against live state first —
  `main` HEAD confirmed at `40244ea` via `git fetch origin main`, `gh api
  .../commits/40244ea... --jq '.files[].filename'` confirms exactly `.nanobots/LEARNINGS.md`,
  matching the docs-only claim. Board, inbox, and PR state all matched cycle 218's report
  exactly (0 inbox, 0 open PRs, no In Progress/In Review/Ready). `main` CI green on `40244ea`;
  both crons healthy (no new red `nanobots-outer.yml` since cycle 218, worker's runs since
  then all green). Pulled cycle 218's own outer-loop run (`34084022172`) denial count via
  `gh run view <id> --log | grep permission_denials_count`: **4**, ordinary band. Posted this
  data point directly as a `gh issue comment` on #19, confirmed landed by reading it back.
- **Lesson:** no change to the standing conclusion — the count's magnitude still hasn't
  correlated with report accuracy in either direction. Continuing to track per RECIPES.md's
  standing instruction until #19's root cause lands.
...
- **Outcome:** n/a (standing metric pull, not a dispatched item; board unchanged — 8/12 Done,
  #18-21 still `summon-human`/Blocked, no maintainer replies on any, checked each issue's
  actual last comment body per RECIPES.md's author-alone-isn't-enough rule)
- **What worked / what didn't:** re-verified cycle 217's report against live state first —
  `main` HEAD confirmed at `6088118` via `git fetch origin main`, `gh api
  .../commits/6088118... --jq '.files[].filename'` confirms exactly `.nanobots/LEARNINGS.md`,
  matching the docs-only claim. Board, inbox, and PR state all matched cycle 217's report
  exactly. `main` CI green on `6088118`; both crons healthy (no new red `nanobots-outer.yml`
  since cycle 217, worker's runs since then all green). Pulled cycle 217's own outer-loop run
  (`34065300960`) denial count via `gh run view <id> --log | grep permission_denials_count`:
  **2**, ordinary band. Posted this data point directly as a `gh issue comment` on #19,
  confirmed landed by reading it back.
- **Lesson:** no change to the standing conclusion — the count's magnitude still hasn't
  correlated with report accuracy in either direction. Continuing to track per RECIPES.md's
  standing instruction until #19's root cause lands.
- **Applies to:** verify
...
- **Outcome:** n/a (standing metric pull, not a dispatched item; board unchanged — 8/12 Done,
  #18-21 still `summon-human`/Blocked, no maintainer replies on any, checked each issue's
  actual last comment body per RECIPES.md's author-alone-isn't-enough rule)
- **What worked / what didn't:** re-verified cycle 205's report (commit `396b734`) against
  live state first — `gh api .../commits/396b734 --jq '.files[].filename'` confirms exactly
  `.nanobots/LEARNINGS.md`, matching the docs-only claim; also confirmed cycle 205's own #19
  comment (cycle 204's denial count = 1) and its #20 recurrence comment both actually landed.
  Pulled cycle 205's own outer-loop run (`33917773779`) denial count via `gh run view <id>
  --log | grep permission_denials_count`: **1**, at the series' established low end (~37
  cycles tracked, 2026-08-26 through 2026-09-04). Sync otherwise fully quiet this cycle: both
  crons healthy (no new red `nanobots-outer.yml` runs since cycle 205's own; worker's recent
  runs all green), no open PRs, no `nanobots:inbox` items, `main` CI green on the current head
  (`396b734`), board unchanged from cycle 205. Posted this data point directly as a `gh issue
  comment` on #19, confirmed landed by reading it back.
- **Lesson:** no change from the prior ~36 data points — the count's magnitude still hasn't
  correlated with report accuracy in either direction. Continuing to track per RECIPES.md's
  standing instruction until #19's root cause lands.
- **Applies to:** verify
...
- **Outcome:** n/a (standing metric pull, not a dispatched item; board unchanged — 8/12 Done,
  ...
  comments)
- **What worked / what didn't:** re-verified cycle 203's report (commit `46701d3`) against
  live state first — `gh api .../commits/46701d3 --jq '.files[].filename'` confirms exactly
  `.nanobots/LEARNINGS.md`, matching the docs-only claim; also confirmed cycle 203's #19
  comment (cycle 202's denial count = 6) actually landed. Pulled cycle 203's own outer-loop
  run (`33837213955`) denial count via `gh run view <id> --log | grep
  permission_denials_count`: **2**, near the series' established low end (~35 cycles
  ...
  current head (`46701d3`), board unchanged from cycle 203. Posted this data point directly
